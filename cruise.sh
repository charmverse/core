...
output_file="depgraphs/${filename_clean}.${now}"
...
depcruise --progress -X "^node_modules"  --output-type dot --ts-config ./tsconfig.json --exclude __tests__  "${filename}" | dot -T svg | depcruise-wrap-stream-in-html > "${output_file}.html"
